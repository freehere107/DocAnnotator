...
>>>>>>> b7cd80b3f1895188d1ae9f9ed3e2f98b464b7f2b
        ...
        (function(root, factory) {
            ...
            function attachDOMEventsToEventBus(eventBus) {
                ...
                eventBus.on('pagechange', function(e) {
                    ...
                    //获取页面浏览的轨迹
                    var scoll_his = localStorage.getItem("scoll_his");
                    if(scoll_his){
                        //获取上一页
                        var i = scoll_his.lastIndexOf(',');
                        if(i == -1){
                            var last_page = 1;
                        }else{
                            var last_page = scoll_his.substr(i+1,scoll_his.length-1);
                        }
                        //设置上一个页面的结束时间
                        setPdfPageTime(last_page+"_end");
                        if(scoll_his == ""){
                            scoll_his = e.pageNumber;
                        }else{
                            scoll_his = scoll_his + "," + e.pageNumber;
                        }
                        localStorage.setItem("scoll_his",scoll_his);
                        //设置当前开始的时间
                        setPdfPageTime(e.pageNumber+"_start");
                        //求取上个页面的阅读时间
                        getPdfPageTime(last_page);
                    }else{
                        localStorage.setItem("scoll_his",e.pageNumber);
                        //设置当前开始的时间
                        setPdfPageTime(e.pageNumber+"_start");
                    }
                    var event = document.createEvent('UIEvents');
                ...
                PDFViewer.prototype = {
                    ...
                    setDocument: function(pdfDocument) {
                        ...
                        return firstPagePromise.then(function(pdfPage) {
                            ...
                            self.eventBus.dispatch('pagesinit', {
                                source: self
                            });
                            if (this.defaultRenderingQueue) {
                                this.update();
